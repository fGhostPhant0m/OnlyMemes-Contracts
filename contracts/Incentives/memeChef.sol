...
// The OnlyMeme's MEMEChef is a fork of 0xDao Garden by 0xDaov1
// The biggest change made from SushiSwap is using per second instead of per block for rewards
// This is due to Sonic extremely fast block times
...
contract GhostChef is Ownable {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken) external onlyOwner {
        ...
        IOnlyMemesPair(address (_lpToken)).setPID(poolId[address(_lpToken)]);
        address tokenA = IOnlyMemesPair(address (_lpToken)).token0();
        address tokenB = IOnlyMemesPair(address (_lpToken)).token1();
        
        if (!Voter.isBribeWhitelisted(tokenA)) {
            Voter.setWhitelistedBribe(tokenA, true);
        }
        if (!Voter.isBribeWhitelisted(tokenB)) { 
            Voter.setWhitelistedBribe(tokenB, true);
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint) external {
        require (address(Voter) == msg.sender , "only Voter can set");
        require(_allocPoint <= MaxAllocPoint, "add: too many alloc points!!");
